...
public class CrudFuncionario implements Crud {
	...
	public void adicionar() {
		while (true) {
			try {
				System.out.print("FUNCIONARIO É PROFESSOR? (S/N) ");
				...
				if (nome.length() < 3)
					throw new IllegalArgumentException("NOME PRECISA TER MAIS QUE 3 CARACTERES");
				System.out.print("INFORME A IDADE: ");
				var idade = input.nextInt();
				if (idade > 65 || idade < 18)
					throw new IllegalArgumentException("IDADE PRECISA ESTAR ENTRE 18 E 65 ANOS");
				input.nextLine();
				...
				if (salario < 1320.00)
					throw new IllegalArgumentException("SALARIO INSERIDO É INVALIDO");
				input.nextLine();
				if (op == 's') {
	...
	@Override
	public void listar() {
		if(listaFunc.size()==0)System.out.println("Não existem funcionarios cadastrados no sistema;");
		else{
			System.out.println("---------------------------------");
	...
	public void remover() {
		if(listaFunc.size()==0) System.out.println("Não existem funcionarios cadastrados no sistema;");
			...
			try {
				...
				if (flag == true) {
					System.out.println("Não foi encontrado nenhum funcionario com esse CPF;");
				}
			} catch(InvalidAttributeValueException e) {
				System.out.println(e.getMessage());
				input.nextLine();
			...
			try {
				...
				for (Funcionario f : listaFunc) {
					if (f.getCpf().equals(cpf)) {
						...
						System.out.println("Informe os dados que serão atualizados, deixe em branco para manter o atual;");
						...
						if (salario < 1320)
							throw new IllegalArgumentException("SALARIO INFORMADO É INVALIDO");
